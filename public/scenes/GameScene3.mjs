...
export class GameScene3 extends BaseScene {
    ...
    createCollision() {
        ...
        const elf1 = this.matter.add.sprite(220 + 32, 1191 + 42, 'elf-dance3', null, {
        ...
        const elf2 = this.matter.add.sprite(339 + 32, 1170 + 42, 'elf-dance4', null, {
            label: `${LABEL_ID.SECOND_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf3 = this.matter.add.sprite(320 + 32, 1340 + 42, 'elf-dance2', null, {
            label: `${LABEL_ID.FIRST_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf4 = this.matter.add.sprite(1450 + 32, 1294 + 42, 'elf-dance5', null, {
            label: `${LABEL_ID.SECOND_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf5 = this.matter.add.sprite(1560 + 32, 1373 + 42, 'elf-dance3', null, {
            label: `${LABEL_ID.FIRST_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf6 = this.matter.add.sprite(1275 + 32, 656 + 42, 'elf-dance1', null, {
            label: `${LABEL_ID.SECOND_KEY}`,
            isStatic: true,
            isSensor: true
        });

        const elf7 = this.matter.add.sprite(1398 + 32, 664 + 42, 'elf-dance2', null, {
            label: `${LABEL_ID.FIRST_JOKE}`,
            isStatic: true,
            isSensor: true
        });
    ...
    createOverlays() {
        const a = myMap.get('answer5');
        const b = myMap.get('answer6');
    ...
    showOverlay() {
        ...
        if (this.eventZone == LABEL_ID.FIRST_KEY) {
            ...
            this.title.setText('Вопрос 5')
            this.title.setVisible(true);
            this.paper.setVisible(true);
            if (this.fold.indexOf('answer5') == -1) {
                this.mySocket.emitAddNewImg('answer5');
            }
        }

        if (this.eventZone == LABEL_ID.SECOND_KEY) {
            this.secondKey.setVisible(true);
            this.title.setText('Вопрос 6')
            this.title.setVisible(true);
            this.paper.setVisible(true);
            if (this.fold.indexOf('answer6') == -1) {
                this.mySocket.emitAddNewImg('answer6');
            }
        }
