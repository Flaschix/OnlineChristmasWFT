...
export class BaseScene extends Phaser.Scene {
    ...
    createFold() {
        this.foldKeys = this.add.image(this.cameras.main.width / 2, this.cameras.main.height / 2 + 10, 'paper');
        ...
        this.foldTitle = this.add.text(570, 260, `Вопрос`, { font: "bold 32px MyCustomFont", fill: '#000000', align: 'center' }).setScrollFactor(0).setDepth(2);
        this.foldTitle.setVisible(false);
        this.foldTitle.setAlpha(1);

        this.foldText = this.add.text(0, 0, `0`, { font: "normal 28px MyCustomFont", fill: '#000000', align: 'center' }).setScrollFactor(0).setDepth(2);
    ...
    moveRightKeys() {
        if (this.foldImgNumber < this.fold.length - 1) {
            ...
            this.tweens.add({
                targets: [this.foldKeys, this.foldText, this.foldTitle],
                alpha: 0,
                duration: 250,
                onComplete: () => {
                    try {
                        this.foldText.setText(decrypt(myMap.get(this.fold[this.foldImgNumber]).text));
                        this.foldTitle.setText(`Вопрос ${myMap.get(this.fold[this.foldImgNumber]).n}`);
