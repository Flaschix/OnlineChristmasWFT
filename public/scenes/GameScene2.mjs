...
export class GameScene2 extends BaseScene {
    ...
    createCollision() {
        ...
        const elf1 = this.matter.add.sprite(370 + 32, 1506 + 42, 'elf-dance3', null, {
        ...
        const elf2 = this.matter.add.sprite(484 + 32, 1485 + 42, 'elf-dance4', null, {
            label: `${LABEL_ID.SECOND_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf3 = this.matter.add.sprite(320 + 32, 1014 + 42, 'elf-dance1', null, {
            label: `${LABEL_ID.SECOND_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf4 = this.matter.add.sprite(430 + 32, 1022 + 42, 'elf-dance2', null, {
            label: `${LABEL_ID.SECOND_KEY}`,
            isStatic: true,
            isSensor: true
        });

        const elf5 = this.matter.add.sprite(1480 + 32, 1525 + 42, 'elf-dance5', null, {
            label: `${LABEL_ID.FIRST_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf6 = this.matter.add.sprite(1590 + 32, 1525 + 42, 'elf-dance2', null, {
            label: `${LABEL_ID.SECOND_JOKE}`,
            isStatic: true,
            isSensor: true
        });

        const elf7 = this.matter.add.sprite(1570 + 32, 1013 + 42, 'elf-dance5', null, {
    ...
    createOverlays() {
        const a = myMap.get('answer3');
        const b = myMap.get('answer4');
        ...
        this.firstKey = this.add.text(a.x, a.y, `${decrypt(a.text)}`, { font: "normal 28px MyCustomFont", fill: '#000000', align: 'center' }).setScrollFactor(0).setDepth(2);
        this.firstKey.setVisible(false);
        this.firstKey.setAlpha(0);

        this.secondKey = this.add.text(b.x, b.y, `${decrypt(b.text)}`, { font: "normal 28px MyCustomFont", fill: '#000000', align: 'center' }).setScrollFactor(0).setDepth(2);
    ...
    showOverlay() {
        ...
        if (this.eventZone == LABEL_ID.FIRST_KEY) {
            ...
            this.title.setText('Вопрос 3')
            this.title.setVisible(true);
            this.paper.setVisible(true);
            if (this.fold.indexOf('answer3') == -1) {
                this.mySocket.emitAddNewImg('answer3');
            }
        }

        if (this.eventZone == LABEL_ID.SECOND_KEY) {
            this.secondKey.setVisible(true);
            this.title.setText('Вопрос 4')
            this.title.setVisible(true);
            this.paper.setVisible(true);
            if (this.fold.indexOf('answer4') == -1) {
                this.mySocket.emitAddNewImg('answer4');
            }
        }
